...
function addToMessageList(userName, imgPath, message, date) {
    ...
    html += `<a class="text-secondary userInput card-link p-0 bg-transparent" href="https://localhost:7280/account/profile?userName=${userName}" asp-controller="Account" asp-action="Profile" asp-route-userName="${userName}">${userName}</a>`;
    html += `<p class="text-secondary"> • ${date}</p>` + `</div>`;
...
document.getElementById("sendButton").addEventListener("click", function (event) {
    ...
    if (userName != null) {
        ...
        connection.invoke("SendComment", userName.textContent, imgPath, itemId, message, date).catch((ex) => {
            console.log(ex)
        })
    }
})